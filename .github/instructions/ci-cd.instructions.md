...
## Workflow Essentials
- Use descriptive workflow names and explicit triggers.
- Set `permissions` explicitly and default to least privilege.
- Use `concurrency` for deploys and shared environments.
- Prefer reusable workflows when duplication appears across pipelines.

## Security
- Prefer OIDC to long-lived cloud credentials.
- Store secrets in GitHub or environment secrets only.
- Pin actions to stable versions, ideally immutable SHAs.
- Add dependency review, SAST, and secret scanning to CI.

## Build & Test Gates
- Run restore, build, lint, and tests before deployment.
- Fail fast on security or quality gates.
- For .NET repos, ensure the pipeline validates the supported SDK and test matrix where relevant.

## Performance & Reliability
- Cache dependencies with keys derived from lockfiles.
- Use matrix jobs only where coverage value justifies runtime cost.
- Pass artifacts cleanly between jobs rather than rebuilding unnecessarily.

## Deployment
- Protect production with environments and approvals.
- Prefer immutable artifacts.
- Surface deployment outputs, rollback path, and health validation.
```yaml
jobs:
  test:
    runs-on: ${{ matrix.os }}
