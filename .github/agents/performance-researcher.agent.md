...
- `planning-research` — when broader investigation plans or option comparison is needed.
- `csharp-engineer` — recommended implementation owner after the report is complete; not before.
- `architecture` — when bottlenecks stem from aggregate boundaries, chatty workflows, or architectural shape.
