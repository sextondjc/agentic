...
| `curate` | Maintain and organise an existing artifact collection | `curating-copilot-instructions` |
| `docker` | Container-scope indicator noun | `docker-dotnet` |

**Noun-form Execution names**: `task-execution`, `branch-completion`, `librarian`, `taxonomy`
...
| Verb / Noun-form | Meaning in context | Example usage |
|---|---|---|
| `audit` | Comprehensive systematic assessment of governance or structure | `audit-governance`, `audit-powershell` |
| `review` | Targeted evaluation of one artifact or set against standards | `skill-review`, `request-code-review`, `remediate-review` |
| `validate` | Confirm an artifact conforms to a schema or contract | `validate-customization` |
...
| `summarize` | Synthesise a large artifact set into a concise briefing | `summarize-governance` |
...
**Noun-form Review names**: `delivery-status-grid`, `current-test-coverage`, `governance-health-overview`, `performance-research`, `security-research`
...
- Review + Governance → `audit-governance`, `validate-customization`
