...
| AGR-S1 | Advisory | `## Specialization` and `## Preferred Companion Skills` present. Missing `## Focus Areas` and `## Standards`. Routing rules and primary rule partially cover these. | Add `## Focus Areas` (routing, intake, scope enforcement) and `## Standards` (reference `lifecycle-governance.instructions.md`). |
...
| AGR-S4 | Pass | `[lifecycle-governance.instructions.md](./../instructions/lifecycle-governance.instructions.md)` — valid path from `.github/agents/`. | Link resolves. |
