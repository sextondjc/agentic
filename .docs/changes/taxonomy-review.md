...
The taxonomy system defines Type, Lane, Domain, and Function layers with explicit machine-readable configuration in [naming-conventions.json](./../.github/skills/taxonomy/references/naming-conventions.json). This is well above average and enables automated audit and deterministic naming during authoring.

### 3. Vocabulary drift is documented, not ignored
Known exceptions (`write-technical-docs`, `delivery-status-grid`, `branch-completion`, `librarian`, `current-test-coverage`) are explicitly documented in [lane-intent.md](./../.github/skills/taxonomy/references/lane-intent.md) with justification. This prevents silent drift from becoming technical debt.
...
[naming-conventions.instructions.md](./../.github/instructions/naming-conventions.instructions.md) is policy-only and routes to authoring skills instead of duplicating procedural detail. This is the correct structural choice.
...
| `keeping-customizations-current` | Renamed → `sync-customizations` |
...
14 of 58 skills (24%) use the `execute-` prefix. The intent was to signal Execution lane ownership via vocabulary, but this produces unbalanced naming: `audit-governance` (Review lane, no prefix) vs. `sync-customizations` (Execution lane, with prefix). Other Execution-lane skills (`build-maui-apps`, `scaffold-dotnet`, `docker-dotnet`, `librarian`) do not use the prefix. The inconsistency reduces discoverability and makes the lane signal unreliable.
...
| R6 | Add `-generator` as an explicit Planning vocabulary noun-form in [function-vocabulary.md](./../.github/skills/taxonomy/references/function-vocabulary.md) | Resolves W6 undocumented suffix | Low | Execution | 🟢 Low |
...
In [naming-conventions.instructions.md](./../.github/instructions/naming-conventions.instructions.md), change:
...
In [naming-conventions.json](./../.github/skills/taxonomy/references/naming-conventions.json), add a `lengthExceptions` array under `scopes.customizations` with a `justification` field per entry. Apply to the two current violations.

### R6 — Add `-generator` to Planning vocabulary (Low)
In [function-vocabulary.md](./../.github/skills/taxonomy/references/function-vocabulary.md), add `-generator` as an accepted noun-form suffix under the Planning table with a note: "Acceptable for skills that produce a structured document artifact as the primary output."
...
|---|---|
| W1 | [naming-conventions.instructions.md](./../.github/instructions/naming-conventions.instructions.md) — `.docs` folder rule |
| W2 | [naming-conventions.instructions.md](./../.github/instructions/naming-conventions.instructions.md) — "File names must reflect the actual content domain" |
| W3 | [function-vocabulary.md](./../.github/skills/taxonomy/references/function-vocabulary.md) — Lane vocabulary consistency |
| W4 | [technical-docs.instructions.md](./../.github/instructions/technical-docs.instructions.md) — missing `.docs/changes` schema |
| W5 | [naming-conventions.instructions.md](./../.github/instructions/naming-conventions.instructions.md) — 12–20 char target |
| W6 | [function-vocabulary.md](./../.github/skills/taxonomy/references/function-vocabulary.md) — Planning vocabulary table |

---

*Report generated by: taxonomy skill (v2.0) + critical-thinking + delivery-status-grid + optimize-customizations*
