# Reference: [architecture-designer.agent.md](./../../../../.github/agents/architecture-designer.agent.md)

## Summary
This page documents one workspace artifact using only file-observed metadata and headings.

## Source
- Path: /[architecture-designer.agent.md](./../../../../.github/agents/architecture-designer.agent.md)
- Documented page: /[architecture-designer.agent.md.ref.md](./architecture-designer.agent.md.ref.md)
- File type: Markdown
- Line count: 144

## Package/Namespace/Module
Not observed in code.

## Syntax or Signature
Not observed in code.

## Parameters
| Name | Value |
|---|---|
| Frontmatter name | architecture |
| Frontmatter applyTo | Not observed in code |

## Return Value
Not observed in code.

## Exceptions and Error Conditions
Not observed in code.

## Remarks
- Primary heading: Architecture & DDD Agent
- Frontmatter description: Unified agent for architectural decision records, domain-driven design guidance, and architectural blueprint synthesis.
- Secondary headings:
- Purpose
- Capabilities
- Workflow
- Context
- Decision
- Consequences
- DDD / SOLID Checklist (Pre-Output)
- ADR Template (Retained)

## Thread Safety and Concurrency Notes
Not observed in code.

## Security Considerations
For policy files, security requirements are documented in their source headings and should be interpreted as standards guidance, not runtime guarantees.

## Performance Notes
Not observed in code.

## Examples
Basic:
- Open the source file and validate metadata and headings against this page.
Advanced:
- Cross-check this page from coverage-report.md to verify documentation traceability.

## See Also
- [index.md](./../../index.md)
- [coverage-report.md](./../../coverage-report.md)
- [code-structure.md](./../../code-structure.md)

